...
namespace repouploader.dev
{
    class devfun
    {
        ...
        public static void showenv()
        {
            ...
            klassen.logger._envlog(1, curr_dir);
        }
                ...
                switch (neuantwort)
                {
                    case "j":
                        ...
                        else
                        {
                            ...
                            if(!File.Exists(klassen.filesystem.appdata + @"\sharploader\env.log"))
                            {
                                Console.ForegroundColor = ConsoleColor.Red;
                                Console.WriteLine("[FEHLER] Konnte die Datei env.log nicht finden...");
                                Console.ResetColor();
                            }
                            else
                            {
                                File.Move(klassen.filesystem.appdata + @"\sharploader\env.log", klassen.filesystem.appdata + @"\sharploader_backup\env.txt");
                            }
                        }
                    ...
                    case "n":
                        ...
                        if (File.Exists(klassen.filesystem.appdata + @"\sharploader\env.log"))
                            File.Delete(klassen.filesystem.appdata + @"\sharploader\env.log");
                        break;
                    default:
                        break;
                }
        ...
        public static void downloadfromlist(string pathtofile, int dlspeed)
        {
            try
            {
                if (pathtofile == string.Empty)
                {
                    ...
                    Console.WriteLine("Bitte geben Sie einen Pfad an!");
                    klassen.logger._logger(1, "PATHTOFILE IS MISSING!");
                ...
                klassen.logger.__logger("\t1", "REMINDER: DLSPEED IS NOT FULLY PROGRAMMED!");

                WebClient client = new WebClient();


                FileInfo _dlfrList = new FileInfo(pathtofile);
                string dlfrlist_name = _dlfrList.Name;
                long dlfrlist_lenght = _dlfrList.Length;
                string dlfrlist_speicherort = _dlfrList.FullName;


                klassen.logger.__logger("\t", "__________________________________________________________");
                klassen.logger.__logger("\t1", "\t\n");
                klassen.logger.__logger("\t1", "Informationen zu PATHTOFILE");
                klassen.logger.__logger("\t1", "Name: " + dlfrlist_name);
                klassen.logger.__logger("\t1", "Lenght:" + dlfrlist_lenght);
                klassen.logger.__logger("\t1", "Speicherort:" + dlfrlist_speicherort);

                StreamReader _dllist = new StreamReader(pathtofile);

                while ((line = _dllist.ReadLine()) != null)
                {
                    klassen.logger.__logger("\t1", "Bearbeite nun Eintrag " + line + " " +  "(" + counter +")" );
                    ...
                    {
                        new_dl = line.Substring(0, 7);
                    }
                    else if (line.StartsWith("https://"))
                    {
                        new_dl = line.Substring(0, 8);
                    }

                    FileInfo download = new FileInfo(line);
                    string name = download.FullName;

                    string downloadverzeichniss = klassen.filesystem.appdata + @"\sharploader\downloads\" + name;

                    switch (line)
                    {
                        case "ftp://":
                             Console.WriteLine("Diese Funktion funktioniert nur für WebAnfragen (HTTP oder HTTPS) bitte benutzen Sie zum runterladen von einem FTP ");
                             Console.WriteLine("Die Funktionen dos() (<- TLS Server) oder do (Für nicht TLS Server)!");

                                klassen.logger.__logger("\t1", "ftp found... aborting");
                            break;
                        case "http://":
                            client.DownloadFile(line, downloadverzeichniss);
                            break;
                        case "https://":
                            break;
                        default:
                    ...
                    klassen.logger.__logger("\t1", "Insgesamt abgearbeitete Links: " + counter);
